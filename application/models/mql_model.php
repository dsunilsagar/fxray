...
var $MQ_CLEAR_NUMBER = 0;
 ...
 function MQ_Query_Register($query) 
 {
   $ret='error';
  ...
  function CreateAccount($user)
     {
      ...
      //--- send request
	 // die();
      return($this->MQ_Query_Register($query));
     }
...
//--- go through all cache directories recursively
         ...
         if ($this->MQ_CLEAR_NUMBER>=T_CLEAR_DELNUMBER) return; // by recursion check condition for function exit 
        }
...
//--- if the file time is expired, delete it and, if the limit of deleted files has been exceeded, exit
         @unlink($fileName);
         if (++$this->MQ_CLEAR_NUMBER>=T_CLEAR_DELNUMBER) return;
         --$size;
        }
     }
...
//---
   ...
   function MQ_Login_Account($login,$password)
  {
   $login = substr($login,0,14);
   $password = substr($password,0,16);
//---
   
   $res = $this->MQ_Query_Trade('USERINFO-login='.$login.'|password='.$password
                  ,T_CACHEDIR
                  ,T_CACHETIME
                  ,$login.'/');
//---
